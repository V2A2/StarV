...
import numpy as np
from StarV.util.load_model import load_beam_model
import time
from StarV.set.probstar import ProbStar
...
def run_beam_model_Krylov(use_arnoldi = None,use_init_space=None):
    ...
    if use_init_space ==True:
        # use output space as projection mat 
        i =  output_space.shape[1] 
    else:
        # use initial sapce as projection mat (transpose dynamics property)
        i = output_space.shape[0]


    unsafe_mat_list = np.array([[-1]])
    unsafe_vec_list =np.array([-500])
    ...
    for hi in h:
        ...
        plot_1D_Star_time(R,time_bound,hi,safety_value= 500 )
    ...
    path = "artifacts/LCS/Beam"
    if not os.path.exists(path):
        os.makedirs(path)

    with open(path+"/Beam.tex", "a") as f:
        if use_init_space == True:
...
if __name__ == '__main__':
    ...
    Xt = run_beam_model_Krylov(use_arnoldi = True,use_init_space=False) 
