...
def generate_exact_reachset_figs(net_id='5x20', show=False):
    'generate 4 pictures and save in ICSE2025/pics/'
    ...
    if net_id=='5x20':
        ...
        plt.savefig(path+"/Figure_6_a__dr_dsafe_vs_vego_Net_{}.png".format(net_id), bbox_inches='tight')  # save figure 6 (a)
        if show: plt.show()

        print('Plot counter output set...')
        plot_probstar_reachset(CO, dir_mat=dir_mat1, dir_vec=dir_vec1, show_prob=False, \
                               label=('$v_{ego}$','$d_r - d_{safe}$'), show=False)
        plt.savefig(path+"/Figure_6_b__counterOutputSet_Net_{}.png".format(net_id), bbox_inches='tight')  # save figure 6 (b)
        if show: plt.show()
        ...
        plt.savefig(path+"/Figure_6_c__counterInitSet_Net_{}.png".format(net_id), bbox_inches='tight')  # save figure 6 (c)
        if show: plt.show()
...
def generate_exact_Q2_verification_results(net_id='5x20'):
    ...
    if net_id == '5x20':
        plt.savefig(path+"/Figure_5_a__Ql_exact_Net_{}.png".format(net_id), bbox_inches='tight')  # save figure [5 (a) N_{5x20}]
    else:
    ...
    plt.legend()
    if net_id == '5x20':
        plt.savefig(path+"/Figure_5_b__Qt_Qt_max_exact_Net_{}.png".format(net_id), bbox_inches='tight')  # save figure [5 (b) N_{5x20}; Appendix N_{3x20}]
    else:
...
def generate_approx_Q2_verification_results(net_id='5x20', pf=0.001):
    ...
    plt.legend()
    if net_id == '5x20':
        plt.savefig(path+"/Figure_5_c__Qt_Qtub_Qt_max_approx_Net_{}.png".format(net_id), bbox_inches='tight')  # save figure [5 (c) N_{5x20}; Appendix N_{3x20}]
    else:
...
def generate_VT_Conv_vs_pf_net():
    ...
    with open(path+"/Table_1__VT_Conv_vs_pf_net.tex", "w") as f:
...
def generate_exact_reachset_figs_AEBS(show=False):
    ...
    for i in range(0, len(initSets)):
        ...
        plt.savefig(path+"/Figure_7_{}__d_vs_vego_init_{}.png".format(chr(i+97), i), bbox_inches='tight')  # save figure 7
        if show: plt.show()
...
def generate_AEBS_Q2_verification_results(initSet_id=0, pf=0.005):
    ...
    plt.yticks(fontsize=13)
    plt.legend()
    plt.savefig(path+"/Figure_8_{}__Qt_exact_Qt_approx_Qt_ub_Qt_max_approx_initSet_{}.png".format(chr(initSet_id+97), initSet_id), bbox_inches='tight')  # save figure 8
...
def generate_AEBS_VT_Conv_vs_pf_initSets():
    ...
    with open(path+"/Table_2__VT_Conv_vs_pf_initSet.tex", "w") as f:
...
if __name__ == "__main__":
    ...
    # verify ACC model
    generate_exact_Q2_verification_results(net_id='3x20') # Appendix
    generate_exact_Q2_verification_results(net_id='5x20') # Fig. 5 (a, b)
    generate_approx_Q2_verification_results(net_id='3x20', pf=0.01) # Appendix
    generate_approx_Q2_verification_results(net_id='5x20', pf=0.01) # Fig. 5 (c)
    #generate_exact_reachset_figs(net_id='3x20')
    generate_exact_reachset_figs(net_id='5x20') # Fig. 6 (a, b, c)
    generate_VT_Conv_vs_pf_net() # Table. 1

    # verify AEBS model
    generate_exact_reachset_figs_AEBS() # Fig. 7
    ...
    generate_AEBS_Q2_verification_results(initSet_id=0, pf=0.005) # Fig. 8 (a)
    generate_AEBS_Q2_verification_results(initSet_id=1, pf=0.005) # Fig. 8 (b)
    generate_AEBS_Q2_verification_results(initSet_id=2, pf=0.005) # Fig. 8 (c)
    generate_AEBS_Q2_verification_results(initSet_id=3, pf=0.005) # Fig. 8 (d)
    generate_AEBS_VT_Conv_vs_pf_initSets() # Table. 2
