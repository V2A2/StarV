"""
RNN training for CMAPSS datatset
Qing Liu, 2/25/2026
"""
import random
...
class RNN_dataset(object):
    ...
    def create_train_input_sequnces(self, train_data, window_size, target_vars=None):
        ...
        Output:
            ...
            y: target selected-sensor vector at the next step after each window (t+1)
        """

        train_data_1 = train_data.copy()

        if target_vars is None:
            target_vars = TARGET_SENSOR_VARS.copy()
        else:
            target_vars = list(target_vars)

        # Use only model features; never use index columns or RUL as model inputs.
        all_features = [c for c in train_data_1.columns if c not in ['unit_number', 'time_cycles', 'RUL']]
        ...
        corr_features_all = [column for column in upper_tri.columns if any(upper_tri[column] > 0.85)]
        # Keep target variables even if they are highly correlated.
        corr_features = [c for c in corr_features_all if c not in target_vars]
        protected_target_corr = [c for c in corr_features_all if c in target_vars]
        print("corr_features (dropped):", corr_features)
        if len(protected_target_corr) > 0:
            print("target vars kept despite high correlation:", protected_target_corr)
        ...
        # skip some constant features, not useful for RUL prediction, not correalted with RUL
        const_vars = []
        for feature in reamining_vars:
            # print(f"Feature {feature} std: {np.std(train_data_1[feature])}")
            # if np.std(train_data_1[feature]) < 2e-3: # if the standard deviation is very small, consider it as constant
            if train_data_1[feature].min() == train_data_1[feature].max():
                if feature in target_vars:
                    raise RuntimeError(f"Target feature {feature} is constant and cannot be predicted.")
                print(f"Feature {feature} has constant values, dropping it.")
        ...
        missing_targets = [v for v in target_vars if v not in selected_vars]
        if len(missing_targets) > 0:
            raise RuntimeError(f"Missing target variables after feature filtering: {missing_targets}")
        ...
        for unit, group in grouped_engine_data:
            ...
            target_cycles = group[target_vars].values   # selected target vars in each engine
            ...
            # predict one-step ahead: window [start, ..., end-1] -> target at end
            for start in range(num_cycles - window_size):
                end = start + window_size
                each_window = engine_cycles[start:end]    
                target_state = target_cycles[end]
                # print("each_window:",each_window)   
                # print(f"target_state_for_cycle{start+window_size+1} in engine{unit}:{target_state}")    
            ...
            if unit in val_engines: # validation set
                ...
                y_val.extend(target_values)
            else:   # training set
                X_train.extend(input_seqs)
                y_train.extend(target_values)


        print(f"number of input seqs for training: {len(X_train)}")
        # print(f"number of input seqs for validation: {len(X_val)}")

        return np.array(X_train), np.array(y_train), np.array(X_val), np.array(y_val), selected_vars, len(selected_vars), target_vars, scaler
    ...
    def create_test_input_sequnces(self,test_data, window_size,selected_vars,scaler):
        ...
        for unit, group in grouped_engine_data:
            ...
            # generate input sequences for each engine
            input_seqs_each_engine =[]

            # build windows for one-step-ahead prediction (target at end index)
            for start in range(num_cycles - window_size):
                end = start + window_size
                each_window = each_engine_cycles[start:end]    
                # print("each_window:",each_window)   
                # print(f"target_state_for_cycle{start+window_size+1} in engine{unit}:{target_state}")    
                input_seqs_each_engine.append(each_window) 
                # engine_ids_per_window.append(unit)

            all_input_seqs_by_engine.append(input_seqs_each_engine)
            # num_windows_per_engine.append(len(input_seqs_each_engine))

            # print(f"number of input seqs for test engine {unit}:{len(input_seqs_each_engine)}")
 
            # if unit in val_engines: # validation set
            #     X_val.extend(input_seqs)
            #     y_val.extend(rul_value)
            # else:   # training set
            X_test.extend(input_seqs_each_engine)
        ...
        return np.array(X_test),all_input_seqs_by_engine,test_data_1
...
class RNN_model(nn.Module):
    def __init__(self, input_size: int, hidden_size:int, fc_sizes:list, rnn_dropout_prob:float, fc_dropout_prob:float, output_size:int):
        ...
        output_layer = nn.Linear(prev_layer, output_size)  # output selected sensor states
...
class RNN_trainer(object):
    def __init__(self, model:nn.Module, train_loader: DataLoader,
                 ...
                 lr: float,weight_decay: float, epochs: int,model_dir:str, selected_vars: list, target_vars: list, scaler: StandardScaler, patience: int):
        self.model = model
    ...
    def train(self):
        ...
        for idx, (x_batch,y_batch) in enumerate(self.train_loader):
            pred_y = self.model(x_batch)
            loss = self.loss_fn(pred_y, y_batch)
            ...
            all_preds.append(pred_y.detach().cpu())
    ...
    def save_model(self):
        ...
        best_model_path = self.model_dir + f"/best_RNN_model_SmoothL1Loss_4_8_win20_h_32_fc_6432_16.pth"
        wait = 0
        train_losses = []
        val_losses = []
...
def pred_last_win_for_each_engine( preds, num_windows):
    ...
    return np.asarray([p[-1] for p in per_engine], dtype=np.float32)
...
if __name__ == "__main__":
    ...
    win_size = 20
    TARGET_SENSOR_VARS = ['var_7', 'var_11', 'var_12','var_15', 'var_20', 'var_21']

    X_train, y_train, X_val, y_val, selected_var, num_features, target_vars, scaler = data.create_train_input_sequnces(
        merged_train_data,
        win_size,
        target_vars=TARGET_SENSOR_VARS
    )
    ...
    print("target_state_shape:",y_train.shape)
    ...
    trainer = RNN_trainer(
        ...
        lr=1e-3, weight_decay=1e-4, epochs=15,
        model_dir=model_dir,
    ...
    np.savez_compressed(model_dir + "/RNN_model_parameters_4_8_win20_h32_f64.npz", **parameters)
    print("Saved model parameters to:", model_dir + "/RNN_model_parameters_4_8_win20_h32_f64.npz")
    ...
    target_vars = checkpoint.get("target_vars", TARGET_SENSOR_VARS)

    _, all_input_sequences,test_scaled = data.create_test_input_sequnces(
        test_data=df_test,
    ...
    target_idx = [selected_vars.index(v) for v in target_vars]
    all_engine_ids = sorted(df_test["unit_number"].unique().tolist())

    all_window_rmse = []
    all_err = []
    engine_avg_rmse = []

    for engine_idx, windows_each_engine in enumerate(all_input_sequences):
        unit_id = all_engine_ids[engine_idx]
        print(f"winow count for engine {unit_id}: {len(windows_each_engine)}")
        if len(windows_each_engine) == 0:
            continue

        X_engine = np.asarray(windows_each_engine, dtype=np.float32)        # (n_windows, window, n_selected)
        engine_cycles = test_scaled[test_scaled["unit_number"] == unit_id].sort_values("time_cycles")[selected_vars].to_numpy(dtype=np.float32)
        expected_n_windows = max(len(engine_cycles) - win_size, 0)

        # one-step-ahead targets for each window of the same engine
        y_engine = engine_cycles[win_size:, :][:, target_idx]              # (n_windows, n_targets)
        print(f"y_engine shape for engine {unit_id}: {y_engine.shape}, target_idx: {target_idx}")
        preds = predict(model, X_engine)                                   # (n_windows, n_targets)
        if preds.shape != y_engine.shape:
            raise RuntimeError(
                f"Prediction/target shape mismatch for engine {unit_id}: preds={preds.shape}, y={y_engine.shape}"
            )

        err = preds - y_engine
        rmse_per_window = np.sqrt(np.mean(np.square(err), axis=1))         # RMSE over target vars per window
        rmse_engine_avg = float(np.mean(rmse_per_window))

        all_window_rmse.extend(rmse_per_window.tolist())
        all_err.append(err)
        engine_avg_rmse.append(rmse_engine_avg)

        if engine_idx < 3:
            print(f"Engine {unit_id}: windows={len(rmse_per_window)}, avg_rmse={rmse_engine_avg:.6f}")

    if len(all_window_rmse) == 0:
        raise RuntimeError("No test windows available for evaluation.")

    all_err = np.vstack(all_err)
    rmse_per_var = np.sqrt(np.mean(np.square(all_err), axis=0))
    rmse_avg_window = float(np.mean(all_window_rmse))          # micro-average over all windows from all engines
    rmse_avg_engine = float(np.mean(engine_avg_rmse))          # macro-average over engines

    print(f"Evaluated engines: {len(engine_avg_rmse)}")
    print(f"Average RMSE across all windows (all engines): {rmse_avg_window:.6f}")
    print(f"Average RMSE across engines (mean of per-engine window RMSE): {rmse_avg_engine:.6f}")
    print("RMSE per target variable (all engines):")
    for v, r in zip(target_vars, rmse_per_var):
        print(f"  {v}: {r:.6f}")

    print(f"Total evaluated windows: {len(all_window_rmse)}")
