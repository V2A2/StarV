...
def prepare_eval_dataset_from_csv(
    ...
    return X_eval, y_eval_raw, len(df)
...
class RNN_trajectory(object):
    ...
    def extract_limo_trajectory_df(self, df: pd.DataFrame):
        new_df = df.copy()
        before = len(new_df)
        # required_cols = list(dict.fromkeys(self.LIMO_INPUT_COLS))
        new_df = new_df.dropna(subset=LIMO_INPUT_COLS).reset_index(drop=True)
    ...
    def process_limo_next_state_dataset(self, processed_traj_path=None):
        ...
        # 1) Scale full input/target trajectory first.
        input_data_scaled = x_scaler.fit_transform(input_data_raw).astype(np.float32)
        target_data_scaled = y_scaler.fit_transform(target_data_raw).astype(np.float32)

        # 2) Save scaled full input trajectory as processed CSV.
        ...
        pd.DataFrame(input_data_raw, columns=self.LIMO_INPUT_COLS).to_csv(X_all_scaled_csv_path, index=False)

        # 3) Build windows from scaled trajectory.
        ...
        # 4) Keep raw validation targets for evaluation.
