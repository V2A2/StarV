...
class RNN_trainer(object):
    def __init__(self, model:nn.Module, train_loader: DataLoader,
        ...
        # Per-state weights tuned from Weighted_Norm_error_1.1_1.5 baseline and
        # then auto-adjusted using validation normalized RMSE each epoch.
        self.state_weight_by_name = {
            "x": 0.75,
            "y": 0.80,
            "yaw": 0.83,
            "v": 1.47,
        ...
        self.state_weights = np.asarray(
            [self.state_weight_by_name.get(v, 1.0) for v in self.selected_vars], dtype=np.float32
        )
        # Auto weight balancing:
        # w_i <- clip((e_i / median(e))^alpha, w_min, w_max), normalized to mean 1,
        # then blended with previous weights by EMA.
        self.enable_auto_weight_tuning = True
        self.auto_weight_start_epoch = 2
        self.auto_weight_update_every = 1
        self.auto_weight_alpha = 0.5
        self.auto_weight_min = 0.6
        self.auto_weight_max = 2.0
        self.auto_weight_ema_keep = 0.6
        self.y_scaler = None
    ...
    def _compute_normalized_error(self, pred_state: torch.Tensor, target_state: torch.Tensor):
        ...
        return error_raw / safe_range

    def _format_state_weights(self):
        return ", ".join(
            [f"{var}={float(self.state_weights[i]):.3f}" for i, var in enumerate(self.selected_vars)]
        )

    def _update_state_weights_from_nrmse(self, nrmse_each: np.ndarray):
        nrmse_each = np.asarray(nrmse_each, dtype=np.float32)
        safe_nrmse = np.clip(nrmse_each, 1e-8, None)
        median_error = float(np.median(safe_nrmse))
        if median_error < 1e-8:
            return self.state_weights.copy(), self.state_weights.copy()

        target_weights = np.power(safe_nrmse / median_error, self.auto_weight_alpha).astype(np.float32)
        target_weights = np.clip(target_weights, self.auto_weight_min, self.auto_weight_max).astype(np.float32)
        target_weights = target_weights / np.clip(target_weights.mean(), 1e-8, None)

        new_weights = (
            self.auto_weight_ema_keep * self.state_weights
            + (1.0 - self.auto_weight_ema_keep) * target_weights
        ).astype(np.float32)
        new_weights = new_weights / np.clip(new_weights.mean(), 1e-8, None)

        self.state_weights = new_weights
        self.state_weight_by_name = {
            var: float(self.state_weights[i]) for i, var in enumerate(self.selected_vars)
        }
        return target_weights, new_weights
   
    def normalized_loss(self, pred_state: torch.Tensor, target_state: torch.Tensor):
        normalized_error = self._compute_normalized_error(pred_state, target_state)

        # Use different metrics by state type and per-state weights.
        state_weights = torch.as_tensor(
            self.state_weights, dtype=normalized_error.dtype, device=normalized_error.device
        )
        total_loss = torch.tensor(0.0, dtype=normalized_error.dtype, device=normalized_error.device)
        total_weight = torch.tensor(0.0, dtype=normalized_error.dtype, device=normalized_error.device)

        for i, var_name in enumerate(self.selected_vars):
            err_i = normalized_error[:, i]
            if var_name in self.velo_var_names:
                state_loss = self.velo_loss_fn(err_i, torch.zeros_like(err_i))
            else:
                state_loss = self.pose_loss_fn(err_i, torch.zeros_like(err_i))
            w_i = state_weights[i]
            total_loss = total_loss + w_i * state_loss
            total_weight = total_weight + w_i

        return total_loss / torch.clamp(total_weight, min=1e-8)
        ...
        with torch.no_grad():
            for idx, (x_batch,y_batch) in enumerate(self.val_loader):
                ...
                normalized_errors.append(
                    self._compute_normalized_error(pred_state, y_batch).detach().cpu().numpy()
                )
                                
        avg_loss = np.mean(losses)
        if len(normalized_errors) == 0:
            nrmse_each = np.ones(len(self.selected_vars), dtype=np.float32)
        else:
            normalized_errors = np.concatenate(normalized_errors, axis=0)
            nrmse_each = np.sqrt(np.mean(normalized_errors ** 2, axis=0)).astype(np.float32)
        return avg_loss, nrmse_each
    ...
    def save_model(self):
        ...
        for e in range(self.epochs):
            ...
            val_loss, val_nrmse_each = self.validate()

            if (
                self.enable_auto_weight_tuning
                and (e + 1) >= self.auto_weight_start_epoch
                and ((e + 1 - self.auto_weight_start_epoch) % self.auto_weight_update_every == 0)
            ):
                target_weights, new_weights = self._update_state_weights_from_nrmse(val_nrmse_each)
                print(
                    "Auto weight update | "
                    f"val nRMSE: {', '.join([f'{v}={val_nrmse_each[i]:.4f}' for i, v in enumerate(self.selected_vars)])} | "
                    f"target w: {', '.join([f'{v}={target_weights[i]:.3f}' for i, v in enumerate(self.selected_vars)])} | "
                    f"used w: {', '.join([f'{v}={new_weights[i]:.3f}' for i, v in enumerate(self.selected_vars)])}"
                )
            ...
            print(f"Epoch {e+1} | "
                        ...
                        f"W [{self._format_state_weights()}]")
            # save best model
            if val_loss < current_val_loss:
                ...
                torch.save({
                    ...
                    "loss_config": {
                        ...
                        "velo_loss": "MSE",
                        "state_weights": {
                            var: float(self.state_weight_by_name.get(var, 1.0))
                            for var in self.selected_vars
                        },
                        "auto_weight_tuning": {
                            "enabled": self.enable_auto_weight_tuning,
                            "start_epoch": self.auto_weight_start_epoch,
                            "update_every": self.auto_weight_update_every,
                            "alpha": self.auto_weight_alpha,
                            "min": self.auto_weight_min,
                            "max": self.auto_weight_max,
                            "ema_keep": self.auto_weight_ema_keep,
                        },
                    },
                    "scaler": self.scaler,
                }, best_model_path)
...
if __name__ == "__main__":
    ...
    print(
        "Training loss: mixed normalized error with per-state weights "
        "(pose x,y,yaw -> SmoothL1, velo v,w -> MSE; "
        "weights auto-balanced by validation nRMSE)"
    )
    ...
    print("Initial state weights:", trainer._format_state_weights())
