...
class RNN_trainer(object):
    ...
    def save_model(self):
        ...
        best_model_path = self.model_dir + f"/best_RNN_model_SmoothL1Loss_4_8_win25_h_32_fc_6432_16.pth"
        wait = 0
        train_losses = []
        val_losses = []
...
if __name__ == "__main__":
    ...
    for name,param_tensor in model.state_dict().items():
        print(name, "\t", model.state_dict()[name])
        parameters[name] = param_tensor.detach().numpy()
    np.savez_compressed(model_dir + "/RNN_model_parameters_4_8_win25_h32_f64.npz", **parameters)
    print("Saved model parameters to:", model_dir + "/RNN_model_parameters_4_8_win25_h32_f64.npz")
    print("Model architecture:", model)
    print("Model parameters:", sum(p.numel() for p in model.parameters()))
