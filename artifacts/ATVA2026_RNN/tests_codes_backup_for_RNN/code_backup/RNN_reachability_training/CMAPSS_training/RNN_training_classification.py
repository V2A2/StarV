...
class RNN_dataset(object):
    ...
    def rul_to_class(self,rul):
        """
        Convert numeric RUL to discrete health class.
        """
        cls = pd.Series(np.zeros(len(rul), dtype=np.int64), index=rul.index)
        cls[(rul >= 0) & (rul < 25)] = 0        # Failure
        cls[(rul >= 25) & (rul < 75)] = 1      # Warning
        cls[(rul >= 75) & (rul < 125)] = 2     # Normal
        cls[(rul >= 125)] = 3                   # Healthy
    ...
    def Merged_with_RUL(self, df_data):
        ...
        df_merged_data['RUL_class'] = self.rul_to_class(df_merged_data['RUL'])
        df_merged_data.drop(columns=['RUL'], inplace=True)
        

        print("\n Merged data with RUL:",df_merged_data.head(120))
        # print("RUL_class counts:",df_merged_data['RUL_class'].value_counts())
    ...
    def create_train_input_sequnces(self,train_data, window_size):
        ...
        for unit, group in grouped_engine_data:
            ...
            rul_class = group["RUL_class"].values                       
            ...
            for start in range(num_cycles - window_size + 1):
                ...
                target_rul_class = rul_class[end-1]
                ...
                rul_classes.append(target_rul_class)
            ...
            if unit in val_engines: # validation set
                ...
                y_val.extend(rul_classes)
            else:   # training set
                X_train.extend(input_seqs)
                y_train.extend(rul_classes)
...
class RNN_model(nn.Module):
    def __init__(self, input_size: int, hidden_size:int, fc_sizes:list,rnn_dropout_prob:float,fc_dropout_prob:float,num_classes:int):
        ...
        self.output_layer = nn.Linear(prev_layer, self.num_classes)  # output RUL

        self.fc_layers.append(self.output_layer)
...
class RNN_trainer(object):
    ...
    def train(self):
        ...
        for idx, (x_batch,y_batch) in enumerate(self.train_loader):
            ...
            print(f"true_class: {y_batch}")

            correct_label += torch.eq(pred_class, y_batch).sum().item()
            total_label += y_batch.size(0)

            # print(f"Batch {idx+1} | Accuracy: {correct/((idx+1)*x_batch.size(0)):.4f}")
            
            all_preds.append(pred_class.detach().cpu())
        ...
        accracy = correct_label/total_label
        # print("Learning rate:", self.optimizer.param_groups[0]['lr'])
        print(f"Train Loss: {avg_loss:.3f} | Train Accuracy: {accracy:.4f}")
        return avg_loss,accracy
    ...
    def validate(self):
        ...
        avg_loss = np.mean(losses)
        accracy = correct_label/total_label
        print(f"Val Loss: {avg_loss:.3f} | Val Accuracy: {accracy:.4f}")
        return avg_loss,accracy
    ...
    def save_model(self):
        ...
        best_model_path = self.model_dir + f"/best_RNN_model_SmoothL1Loss_classification.pth"
        wait = 0
        train_losses = []
        val_losses = []
        train_accuracies = []
...
def predict(model, X_test, batch_size=256):
    ...
    with torch.no_grad():
        for (x_batch,) in dl:
            ...
            #  print("preds_shape:",pred_logits.shape)   
            pred_class = torch.argmax(pred_logits, dim=1)  
            preds_classes.append(pred_class.detach().cpu().numpy())

    return np.concatenate(preds_classes)



def pred_last_win_for_each_engine( preds_classes, num_windows):

    preds_per_engine = []
    idx = 0

    for n in num_windows:
        last_pred = preds_classes[idx + n - 1]  # last window
        preds_per_engine.append(last_pred)
        idx += n

    return np.array(preds_per_engine)
...
if __name__ == "__main__":
    ...
    model= RNN_model(input_size=num_features, hidden_size=32, fc_sizes=[64, 32, 16], rnn_dropout_prob=0.2, fc_dropout_prob=0.2,num_classes=4)
    # model.init_weights()
    ...
    save_model_path, train_losses, val_losses,train_accuracies,val_accuracies = trainer.save_model()
    ...
    # Predict on test set
    preds_classes= predict(model, X_test)
    pred_for_engine = pred_last_win_for_each_engine(preds_classes, num_win_per_engine)
    print("pred_for_engine:",pred_for_engine)
    true_test_rul_class = data.rul_to_class(y_test["RUL"]).values.reshape(-1)
    print("true_test_rul_class:",true_test_rul_class)
    correct_test = np.sum(pred_for_engine == true_test_rul_class)
    total_test = len(true_test_rul_class)
