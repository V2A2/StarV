...
class PosLin(object):
    ...
    def stepReach(*args):
        ...
        if xmin >= 0:
            ...
            # print("get exact xmin  and exact xmax by LP solver")
            xmax = I.getMax(index, lp_solver)
            if xmax <= 0:
                # print("exact max <=0, reset")
                S = []
                S.append(I.resetRow(index))
                # print("Set after stepReach:", I)
            else:
                ...
                if xmin >= 0:
                    # print("exact min >=0, keep set")
                    S = []
                    S.append(I)
                    # print("Set after stepReach:", I)
                else:
                    # print("exact xmin < 0 and exact xmax > 0, need to split the set")
                    ...
                    # for i in range(len(S)):
                        # print("Set {}: {}".format(i, S[i]))
            # print("Number of sets after splitting:", len(S))
        # for i in range(len(S)):
        #     print("Set {}: {}".format(i, S[i]))
        # print(f"number of input sets after check index {index}: {len(S)}")
        # print("=============End stepReach=================\n")
        return S
    ...
    def reachExactSingleInput(*args):
        ...
        for i in range(0, In.dim):
            ...
            # print(f"number of input sets after check index {i}: {len(S1)}")
