...
def build_windows(
    ...
    if mode not in {"sliding", "expanding_prefix", "block_expanding_prefix"}:
        raise RuntimeError(f"Unsupported window mode: {mode}")
    ...
    if mode == "sliding":
        ...
        y_out = np.array(y, dtype=np.float32)
        idx_out = np.asarray(target_indices, dtype=np.int64)
        return (X_out, y_out, idx_out) if return_target_indices else (X_out, y_out)

    if mode == "expanding_prefix":
        # Global expanding-prefix windows:
        # step (window_size+1) uses prefix [0:window_size]
        # step (window_size+2) uses prefix [0:window_size+1], etc.
        for end in range(window_size, input_data.shape[0]):
            X.append(input_data[:end, :])
            y.append(target_data[end, :])
            target_indices.append(end)
        X_out = _as_float32_windows(X)
        y_out = np.array(y, dtype=np.float32)
        idx_out = np.asarray(target_indices, dtype=np.int64)
        return (X_out, y_out, idx_out) if return_target_indices else (X_out, y_out)

    # Block-based expanding-prefix windows:
    # for each fixed block [start, start+block_size), generate prefixes
    # [start:start+window_size], ..., [start:start+block_size-1]
    # and predict next states at end indices.
    # This yields exactly (block_size - window_size) predictions per block.
    if block_size is None or block_stride is None:
        raise RuntimeError(
            "block_expanding_prefix requires block_size and block_stride."
        )
    blk = int(block_size)
    stride = int(block_stride)
    min_prefix = int(window_size)
    if blk <= 0 or stride <= 0 or min_prefix <= 0:
        raise RuntimeError(
            f"Invalid block parameters: window_size={min_prefix}, "
            f"block_size={blk}, block_stride={stride}"
        )
    if blk <= min_prefix:
        raise RuntimeError(
            f"block_size ({blk}) must be > window_size ({min_prefix}) "
            "to produce at least one next-state prediction."
        )
    if input_data.shape[0] < blk:
        raise RuntimeError(
            f"Not enough steps ({input_data.shape[0]}) for block_size {blk}."
        )

    n_blocks = 0
    for block_start in range(0, input_data.shape[0] - blk + 1, stride):
        n_blocks += 1
        block_end = block_start + blk
        for end in range(block_start + min_prefix, block_end):
            X.append(input_data[block_start:end, :])
            y.append(target_data[end, :])
            target_indices.append(end)

    if n_blocks == 0:
        raise RuntimeError(
            f"No valid blocks for block_size={blk}, stride={stride}, steps={input_data.shape[0]}."
        )
...
class RNN_trajectory(object):
    ...
    def build_next_state_windows(
        ...
        win = self.window_size if window_size is None else int(window_size)
        if win <= 0:
            raise RuntimeError(f"Invalid window size: {win}")
        if input_data.shape[0] <= win:
            raise RuntimeError(
                f"Not enough steps ({input_data.shape[0]}) for window size {win}"
            )
        return build_windows(
            ...
            return_target_indices=return_target_indices,
        )
    ...
    def train_test_win_split(self, X_all: np.ndarray, y_all: np.ndarray):
        ...
        print(f"Train/Val split: {len(X_train)} train windows, {len(X_val)} val windows.")
    ...
    def process_limo_next_state_dataset(self, processed_traj_path=None):
        ...
        print(f"number of input trajeectory states: {len(input_data_raw)}, number of target trajectory states: {len(target_data_raw)}")
        ...
        X_all, y_all, y_target_indices = self.build_next_state_windows(
            input_data_scaled,
            target_data_scaled,
        ...
        y_all_raw = target_data_raw[y_target_indices].astype(np.float32)
        ...
        return {
            ...
            "target_data_scaled": target_data_scaled.astype(np.float32),
            "window_size": int(self.window_size),
            "window_mode": self.window_mode,
            "block_size": self.block_size,
            "block_stride": self.block_stride,
            "y_target_indices": y_target_indices.astype(np.int64),
            "X_train": _as_float32_windows(X_train_processed),
            "y_train": y_train_processed.astype(np.float32),
            "X_val": _as_float32_windows(X_val_processed),
...
class LIMO_Dataset(Dataset):
    def __init__(self, X, y=None):
        self.variable_length = isinstance(X, list)
    ...
    def __getitem__(self, idx):
        ...
        return inputs, self.y[idx], seq_len


def limo_collate_fn(batch):
    has_target = len(batch[0]) == 3
    if has_target:
    ...
    if has_target:
        return padded_x, ys_tensor, lengths
...
class RNN_model(nn.Module):
    ...
    def forward(self, x, lengths: Optional[torch.Tensor] = None):
        ...
        if lengths is not None:
            ...
            _, h = self.rnn(packed_x)
        else:
            _, h = self.rnn(x)
        # output: tensor of shape (L,D∗Hout)(L,D∗Hout​)  h: tensor of shape (D*num_layers,Hout​), D = 2 if bidirectional=True otherwise 1
...
class RNN_trainer(object):
    def __init__(self, model:nn.Module, train_loader: DataLoader,
                 ...
                 window_schedule: Optional[list] = None,
                 input_data_scaled: Optional[np.ndarray] = None,
                 target_data_scaled: Optional[np.ndarray] = None,
        ...
        self.window_loader_cache = {}
        self.epoch_window_schedule = None
        self.current_train_window = None
        ...
        # loss function and loss weights for states
        ...
        # Optional curriculum: increase window size across epochs while keeping
        # a fixed validation window (typically the final deployment window).
        if window_schedule is not None:
            if input_data_scaled is None or target_data_scaled is None:
                raise RuntimeError(
                    "window_schedule requires input_data_scaled and target_data_scaled."
                )
            if batch_size is None:
                raise RuntimeError("window_schedule requires batch_size.")
            if val_ratio is None:
                raise RuntimeError("window_schedule requires val_ratio.")

            self.epoch_window_schedule = self._expand_window_schedule(window_schedule)
            if self.eval_window_size is None:
                self.eval_window_size = int(self.epoch_window_schedule[-1])
            self.current_train_window = int(self.epoch_window_schedule[0])

            self.train_loader, _ = self._get_loaders_for_window(self.current_train_window)
            _, self.val_loader = self._get_loaders_for_window(int(self.eval_window_size))
            print(
                f"Enabled incremental window training: schedule={self.epoch_window_schedule} "
                f"| val_window={self.eval_window_size}"
            )

    def _expand_window_schedule(self, window_schedule: list):
        if not isinstance(window_schedule, list) or len(window_schedule) == 0:
            raise RuntimeError("window_schedule must be a non-empty list of positive integers.")
        wins = [int(w) for w in window_schedule]
        if any(w <= 0 for w in wins):
            raise RuntimeError(f"Invalid window schedule: {window_schedule}")

        # If one or more window sizes per epoch are provided, use first `epochs`.
        if len(wins) >= self.epochs:
            return wins[: self.epochs]

        # Otherwise distribute training epochs across schedule stages.
        n_stages = len(wins)
        base_len = self.epochs // n_stages
        rem = self.epochs % n_stages
        epoch_schedule = []
        for i, w in enumerate(wins):
            stage_len = base_len + (1 if i < rem else 0)
            epoch_schedule.extend([w] * stage_len)
        if len(epoch_schedule) < self.epochs:
            epoch_schedule.extend([wins[-1]] * (self.epochs - len(epoch_schedule)))
        return epoch_schedule[:self.epochs]

    def _get_loaders_for_window(self, window_size: int):
        win = int(window_size)
        if win in self.window_loader_cache:
            return self.window_loader_cache[win]

        X_all, y_all = build_windows(
            self.input_data_scaled,
            self.target_data_scaled,
            win,
            mode=self.window_mode,
            block_size=self.block_size,
            block_stride=self.block_stride,
        ...
        if split_idx <= 0 or split_idx >= len(X_all):
            raise RuntimeError(
                f"Invalid split index {split_idx} for {len(X_all)} windows at window size {win}."
            )

        X_train = _as_float32_windows(X_all[:split_idx])
        y_train = y_all[:split_idx].astype(np.float32)
        X_val = _as_float32_windows(X_all[split_idx:])
        y_val = y_all[split_idx:].astype(np.float32)
        print(
            f"Window {win}: train windows={len(X_train)}, val windows={len(X_val)}"
        )

        train_loader = DataLoader(
            LIMO_Dataset(X_train, y_train),
            batch_size=self.batch_size,
            shuffle=True,
            collate_fn=limo_collate_fn,
        )
        val_loader = DataLoader(
            LIMO_Dataset(X_val, y_val),
            batch_size=self.batch_size,
            shuffle=False,
            collate_fn=limo_collate_fn,
        )
        self.window_loader_cache[win] = (train_loader, val_loader)
    ...
    def normalized_loss(self, pred_state: torch.Tensor, target_state: torch.Tensor):
        ...
        # Use different loss metrics for pose states(x,y,yaw) and velocity states (v and w).
    ...
    def train(self):
        ...
        all_preds=[]
        all_targets =[]
        for idx, (x_batch, y_batch, seq_lens) in enumerate(self.train_loader):
            # print(type(x_batch))
            # print(len(x_batch))
            ...
            pred_state = self.model(x_batch, seq_lens)
            # print("pred_state_LIMO:",pred_state)
        ...
        print(f"all_training_preditions_shape:{torch.cat(all_preds, dim=0).shape}, all_training_targets_shape:{torch.cat(all_targets, dim=0).shape}")
    ...
    def validate(self):
        ...
        all_targets =[]
        with torch.no_grad():
            for idx, (x_batch, y_batch, seq_lens) in enumerate(self.val_loader):
                # x_batch = x_batch.to(self.device)
                # y_batch = y_batch.to(self.device)
                pred_state = self.model(x_batch, seq_lens)
                loss = self.normalized_loss(pred_state, y_batch)
    ...
    def save_model(self):
        ...
        best_model_path = self.model_dir + "/best_RNN_next_state_model_expand.pth"
        wait = 0
        train_losses = []
        val_losses = []
        for e in range(self.epochs):
            if self.epoch_window_schedule is not None:
                win = int(self.epoch_window_schedule[e])
                if self.current_train_window != win:
                    self.train_loader, _ = self._get_loaders_for_window(win)
                    self.current_train_window = win
                print(
                    f"======== Epoch {e+1}/{self.epochs} "
                    f"(train_window={win}, val_window={self.eval_window_size}) ========"
                )
            else:
...
def predict(model, X_test, batch_size=32):
    model.eval()
    ds = LIMO_Dataset(X_test, y=None)
    dl = DataLoader(ds, batch_size=batch_size, shuffle=False, collate_fn=limo_collate_fn)
    preds = []
    with torch.no_grad():
        for x_batch, seq_lens in dl:
            p = model(x_batch, seq_lens).detach().cpu().numpy()
            preds.append(p)
...
def plot_loss_curve(train_losses, val_losses,save_path="./"):
    ...
    plt.legend()
    plt.grid()
    plt.savefig(save_path + '/loss_curve_RNN_expand.png')
    plt.show()
    # plt.close()
...
def plot_prediction_vs_origin_over_time(
    ...
    out1 = os.path.join(save_path, "pred_vs_origin_over_time_expand.png")
    ...
    out2 = os.path.join(save_path, "pred_error_over_time_expand.png")
...
def evaluate_next_state_with_normalized_error(
    ...
    if isinstance(X_val, list):
        print(
            f"X_val: {len(X_val)} variable-length windows "
            f"(min_len={min(len(x) for x in X_val)}, max_len={max(len(x) for x in X_val)})"
        )
    else:
...
if __name__ == "__main__":
    ...
    batch_size = 32
    epochs = 100
    # "sliding": fixed-length rolling windows
    # "expanding_prefix": step k uses all previous prefix steps [0:k)
    # "block_expanding_prefix": per 40-step block with stride 20,
    #   use prefixes 20..39 to predict 21..40 relative to each block start
    #   (exactly 20 predictions per block).
    window_mode = "block_expanding_prefix"
    use_incremental_window = False
    # For optional curriculum, start from this minimum prefix length.
    start_window_size = win_size
    ...
    eval_window_size = win_size
    if use_incremental_window:
        if window_mode == "expanding_prefix":
            max_feasible_window = max(win_size, data["input_data_scaled"].shape[0] - 2)
            schedule_end_window = min(max_feasible_window, start_window_size + epochs - 1)
            window_schedule = list(range(start_window_size, schedule_end_window + 1))
        elif window_mode == "block_expanding_prefix":
            schedule_end_window = min(block_size - 1, start_window_size + epochs - 1)
            window_schedule = list(range(start_window_size, schedule_end_window + 1))
        else:
            window_schedule = list(range(start_window_size, win_size + 1))
        eval_window_size = int(window_schedule[-1])
    ...
    print("Window size (minimum):", win_size)
    print("Window mode:", window_mode)
    if window_mode == "block_expanding_prefix":
        print("Block size:", block_size, "| Block stride:", block_stride)
        print(
            "Predictions per block:",
            block_size - win_size,
            "(non-overlapping target indices when stride=20).",
        )
    if window_schedule is not None:
        if len(window_schedule) <= 25:
            print("Incremental train window schedule:", window_schedule)
        else:
            print(
                "Incremental train window schedule:"
                f" [{window_schedule[0]}..{window_schedule[-1]}] "
                f"({len(window_schedule)} total stages)"
            )
        print("Validation/evaluation window size:", eval_window_size)
    if isinstance(X_train, list):
        train_lens = [len(x) for x in X_train]
        val_lens = [len(x) for x in X_val]
        print(
            "Train windows:",
            f"{len(X_train)} samples (len {min(train_lens)}..{max(train_lens)})",
            "Targets:",
            y_train.shape,
        )
        print(
            "Val windows:",
            f"{len(X_val)} samples (len {min(val_lens)}..{max(val_lens)})",
            "Targets:",
            y_val.shape,
        )
    else:
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=batch_size, shuffle=True, collate_fn=limo_collate_fn
    )
    val_loader = DataLoader(
        val_dataset, batch_size=batch_size, shuffle=False, collate_fn=limo_collate_fn
    )
    ...
    trainer = RNN_trainer(
        ...
        input_data_scaled=data["input_data_scaled"],
        target_data_scaled=data["target_data_scaled"],
    ...
    np.savez_compressed(model_dir + "/RNN_model_parameters_LIMO_with_weights_1_expand.npz", **parameters)
    print("Saved model parameters to:", model_dir + "RNN_model_parameters_LIMO_with_weights_1_expand.npz")
