"""
Main Verifier Class
Bryan Duong, 9/18/2024
(Edit quantiVerifyExactBFS for RNN)
"""
...
def checkSafetyStar(unsafe_mat, unsafe_vec, S):
    ...
    ), "error: constraint vector \
    should be a 1D numpy array"
    assert (
        C.shape[0] == d.shape[0]
    ), "error: inconsistency between constraint matrix and \
    constraint vector"
...
def quantiVerifyExactBFS(net, inputSet, unsafe_mat, unsafe_vec, lp_solver="gurobi", numCores=1, show=True):
    ...
    if pool is None and net.type == "rnn":
        for Sn in S:
            for S1 in Sn:
