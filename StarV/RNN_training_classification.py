...
def predict(model, X_test, batch_size=256):
    model.eval()
    ds = CMAPSS_Dataset(torch.tensor(X_test, dtype=torch.float32))
...
if __name__ == "__main__":
    ...
    # Predict on test set
    preds_logits = predict(model, X_test)
    print("preds_shape:",preds_logits.shape)   
    pred_class = torch.argmax(preds_logits, dim=1)  
    pred_for_engine = pred_last_win_for_each_engine(pred_class, num_win_per_engine)
