...
class IdentityXIdentity(object):
    ...
    def getConstraints(xl, xu, yl, yu):
        ...
        if z_zmax == 0 or z_zmax == 1:
            #splitted
            Ux = np.row_stack([gammaU, betaU])
            Uy = np.row_stack([alphaU, muU])
            Ub = Ux.flatten()*pzs[z_max, x_] + Uy.flatten()*pzs[z_max, y_] - pzs[z_max, z_]

        elif z_zmax == 2 or z_zmax == 3:
            Ux = np.row_stack([gammaU, betaU])
            Uy = np.row_stack([muU, alphaU])
            Ub = Ux.flatten()*pzs[z_max, x_] + Uy.flatten()*pzs[z_max, y_] - pzs[z_max, z_]
        ...
        if  z_zmin == 0 or z_zmin == 1:
            #splitted
            Lx = np.row_stack([gammaL, betaL])
            Ly = np.row_stack([muL, alphaL])
            ...
            Lx = np.row_stack([gammaL, betaL])
            Ly = np.row_stack([alphaL, muL])
            Lb = Lx.flatten()*pzs[z_min, x_] + Ly.flatten()*pzs[z_min, y_] - pzs[z_min, z_]
    ...
    def reachApprox_sparse(X, H, lp_solver='gurobi', RF=0.0, DR=0):
        ...
        n = X.dim
        S = X.minKowskiSum(H)
        return IdentityXIdentity.multiStep_sparse(S, X, H, lp_solver=lp_solver, RF=RF, DR=DR)
