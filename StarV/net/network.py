...
def reachExactBFS(net, inputSet, lp_solver="gurobi", pool=None, show=True):
    ...
    if net.type == "rnn" and net.n_layers >= 2:
        S_next_layer = S
        ...
        for i in range(net.n_layers):
            ...
            if i == 0:
                ...
                output_set, hidden_sets = net.layers[i].reach(S_prev_layer, method="exact", lp_solver=lp_solver, pool=pool)
            else:
                # Compute reachable set for other layers
                hidden_sets = []
                outputset = []
                for hidden_prev_layer in S_prev_layer:
                    output, hidden_set = net.layers[i].reach(hidden_prev_layer, method="exact", lp_solver=lp_solver, pool=pool)
                ...
                # Arrange output set for the last layer
                if i == net.n_layers - 1:
                    output_set = [[output[i][0] for output in outputset] for i in range(len(outputset[0]))]
