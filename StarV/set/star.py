...
class Star(object):
    ...
    def __init__(self, *args):
        ...
        if len(args) == 5:
            ...
            if len(C) != 0:
                ...
                assert (
                    C.shape[1] == pred_lb.shape[0] and C.shape[1] == pred_ub.shape[0]
                ), "error: \
            ...
            assert (
                lb.shape[0] == ub.shape[0]
            ), "error: \
            inconsistency between predicate lower bound and upper bound"
            if np.any(ub < lb):
                raise RuntimeError(
                    "Upper bound (ub) must be greater or equal the lower bound (lb) for all D dimensions!"
                )
            ...
            self.C = np.array([])
            self.d = np.array([])
            self.pred_lb = -np.ones(
                nVars,
            )
            self.pred_ub = np.ones(
                nVars,
            )
            self.nVars = nVars
    ...
    def estimateRanges(self):
        ...
        lb = c1 + np.matmul(v1, self.pred_ub) + np.matmul(v2, self.pred_lb)
        ub = c1 + np.matmul(v1, self.pred_lb) + np.matmul(v2, self.pred_ub)
    ...
    def minKowskiSum(self, Y):
        ...
        assert (
            self.dim == Y.dim
        ), "error: inconsistent dimension between the input and the self object"
    ...
    def updatePredicateRanges(cls, newC, newd, pred_lb, pred_ub):
        ...
        assert (
            isinstance(newC, np.ndarray) and len(newC.shape) == 1
        ), "error: \
            new constraint matrix should be 1D numpy array"
        assert (
            isinstance(newd, np.ndarray) and len(newd.shape) == 1 and newd.shape[0] == 1
        ), "error: \
            new constraint vector should be 1D numpy array"
        assert (
            isinstance(pred_lb, np.ndarray) and len(pred_lb.shape) == 1
        ), "error: \
            lower bound vector should be 1D numpy array"
        assert (
            isinstance(pred_ub, np.ndarray) and len(pred_ub.shape) == 1
        ), "error: \
            upper bound vector should be 1D numpy array"
        assert (
            pred_lb.shape[0] == pred_ub.shape[0]
        ), "error: \
        inconsistency between the lower bound and upper bound vectors"
        assert (
            newC.shape[0] == pred_lb.shape[0]
        ), "error: \
    ...
    def addConstraint(self, C, d):
        ...
        assert (
            isinstance(C, np.ndarray) and len(C.shape) == 1
        ), "error: \
        ...
        if len(self.C) != 0:
            ...
            self.d = newd
        new_pred_lb, new_pred_ub = Star.updatePredicateRanges(
            newC, newd, self.pred_lb, self.pred_ub
        )
    ...
    def addMultipleConstraints(self, C, d):
        ...
        assert isinstance(
            C, np.ndarray
        ), "error: constraint matrix should be a numpy array"
        assert isinstance(
            d, np.ndarray
        ), "error: constraint vector should be a numpy array"
        assert (
            C.shape[0] == d.shape[0]
        ), "error: inconsistency between \
